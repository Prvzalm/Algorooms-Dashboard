...
const StrategyBuilder = () => {
  ...
  const handleAddInstrument = () => {
    const dummyInstrument = "NIFTY";
    if (!selectedInstruments.includes(dummyInstrument)) {
      setSelectedInstruments([...selectedInstruments, dummyInstrument]);
    }
  };
              ...
              {selectedInstruments.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-2">
                  {selectedInstruments.map((instrument, idx) => (
                    <span
                      key={idx}
                      className="bg-blue-100 text-blue-600 px-2 py-1 rounded text-xs break-words dark:bg-[#1E2027] dark:text-blue-400"
                    >
                      {instrument}
                    </span>
                  ))}
